"""
Test uni-axial compression without gravity
"""
...
from fenics import plot as fplot
...
bed_slope =   0.0
strain_rate = 1e-16
left_vel =    length*strain_rate/2#/material.secpera*material.time_factor
right_vel =  -length*strain_rate/2#/material.secpera*material.time_factor
...
#p.change_property(1e-2*np.random.rand(len(pstrain)),1)
#_____________________________________________
...
model.friction = 1e-32#/(surf_fun(0)-bed_fun(0))/model.rho_i/model.g # Friction coefficient
...
max_length = length# Regrid if length exceeds this value
min_length = length
...
fname_dir = 'data/calibrate/'
fname_base = fname_dir + 'calibration_1_'+str((left_vel-right_vel)/length)+'_T_'+str(Tb)+'/'
...
if save_files==True:
    import shutil
    shutil.copy2('calibrate.py', fname_base+'calibrate.py')
...
model.right_wall = length
outF = open(fname_base+"calibrate3.txt", "w")
strain_rate = (left_vel-right_vel)/length
s = []
f1 = []
f2 = []
model.g =1e-256
...
for i in range(i,1000):
   #model.left_vel =    length*strain_rate/2#/material.secpera*material.time_factor
   #model.right_vel =  -length*strain_rate/2#/material.secpera*material.time_factor
   ...
   sigma = 2 * model.eta * 0.5*(nabla_grad(u) + nabla_grad(u).T) - pres*Identity(2)
   n = FacetNormal(model.mesh.mesh)
   Tn = dot(dot(sigma,n),n)
   Tn = dot(sigma,n) # Normal stress
   flux_1 = assemble(Tn[0]*model.DS(3))/assemble(dot(n,n)*model.DS(3))
   flux_2 = assemble(Tn[0]*model.DS(4))/assemble(dot(n,n)*model.DS(4))
   #flux_3 = assemble(Tn*model.DS(1))/assemble(dot(n,n)*model.DS(1))
   flux_3 = 0.0
   flux_4 = assemble(Tn[1]*model.DS(5))/assemble(dot(n,n)*model.DS(5))
   print('Flux 4',flux_4/1e6)
   eta_visc_2 = assemble(model.eta_visc*model.DS(4))/assemble(dot(n,n)*model.DS(4))
   eta_visc_1 = assemble(model.eta_visc*model.DS(3))/assemble(dot(n,n)*model.DS(3))
   #eta_visc=assemble(model.eta_visc*dx)/assemble(interpolate(Constant(1.0),Q0)*dx)
   sigma1=(flux_1-flux_2)/1e6
   sigma2=(flux_3-flux_4)/1e6
   sigma11 = -0.5*(sigma1+sigma2)
   sigma11 = sigma1
   sigma12 = 0.5*(sigma1-sigma2)

   #sigma11=-(flux_1-flux_2)/1e6
   #sigma12=(flux_3-flux_4)/1e6

   tau_xx = 2 * model.eta * 0.5*(nabla_grad(u) + nabla_grad(u).T)
   epsII_visc=np.abs(assemble(sqrt(tau_xx[0,0]**2+tau_xx[0,1]**2)/model.eta_visc*dx)/assemble(interpolate(Constant(1.0),Q0)*dx))
   strain_rate = (model.left_vel-model.right_vel)/(model.right_wall-model.left_wall)
   accumulated_strain = accumulated_strain + np.maximum(strain_rate-epsII_visc,0.0)*time_step
   line = str(accumulated_strain)+','+str(strain_rate/secpera)+','+str(sigma11)+','+str(sigma12)+','+str(np.max(pstrain))+'\n'
   outF.write(line)
   s.append(accumulated_strain)
   f1.append(sigma11)
   f2.append(sigma12)
   strain_rate_vec.append(strain_rate/secpera)
   #model.right_wall = model.right_wall + model.right_vel*time_step
   #model.left_wall = model.left_wall + model.left_vel*time_step
   ...
   plt.subplot(2,2,1)
   c=plot(u);plt.colorbar(c)
   ax=plt.subplot(2,2,2)
   c=plt.scatter(xp[:,0],xp[:,1],s=1.0,c=np.log10(pepsII/material.time_factor+1e-16),vmin=-8,vmax=-5);cbar2=plt.colorbar(c);plt.axis('equal')
   #plt.xlim([0,length])
   #plt.ylim([0,length])
   ax.set_aspect('equal',adjustable='box')

   ax=plt.subplot(2,2,3)
   c=plt.scatter(xp[:,0],xp[:,1],s=1.0,c=np.log10(np.maximum(pstrain,1e-16)),vmin=-4,vmax=1);cbar1=plt.colorbar(c);
   #plt.xlim([0,length])
   #plt.ylim([0,length])
   ax.set_aspect('equal',adjustable='box')
   ...
   #plt.xlim([])
   plt.pause(1e-16);
   print('Time step',time_step,'Stress',sigma11,'Shear',sigma12,'Percent yielded',glenVisc.num_yielded,'Maximum strain',np.max(pstrain))
   ...
   if t>1.01:
       break
   if glenVisc.num_yielded>0.0:
       strain_rate = strain_rate + time_step*10.0
       model.right_vel = -strain_rate*(model.right_wall-model.left_wall)/2
       model.left_vel =   strain_rate*(model.right_wall-model.left_wall)/2
   else:
       strain_rate = strain_rate + time_step*10.0
       model.right_vel = -strain_rate*(model.right_wall-model.left_wall)/2
       model.left_vel =   strain_rate*(model.right_wall-model.left_wall)/2
   x=model.mesh.mesh.coordinates()
   print('Right wall',model.right_wall,'Mesh wall',np.max(x[:,0]))

   #strain_rate = strain_rate+ strain_rate_accel
